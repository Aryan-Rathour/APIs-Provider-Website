...
export function MagicCard({
  ...
  return (
    ...
    >
      ...
      />
      {/* Apply gradientColor to border */}
      <motion.div
        className="pointer-events-none absolute inset-0 rounded-xl border-4 border-solid transition-all duration-300 group-hover:border-8"
        style={{
          background: useMotionTemplate`
            radial-gradient(${gradientSize}px circle at ${mouseX}px ${mouseY}px, ${gradientFrom}, ${gradientTo}, hsl(var(--border)) 100%)
          `,
          borderColor: gradientColor, // Apply gradientColor to border
        }}
      />
    </div>
  );
